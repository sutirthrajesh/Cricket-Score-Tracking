...
from tkinter import messagebox, filedialog   #MODIFIED
...
class CricketScoreTracker:
    def __init__(self, root):
        ...
        #ADDED: log ball by ball
        self.ball_log = [[], []]  # each ball appended as int (given from last score) or "W" for wicket
    ...
    def open_batting_screen(self):
        ...
        self.batting_window.config(bg="white")
    ...
    def add_runs(self, runs):
        ...
        # ADDED
        self.ball_log[t].append(str(runs))
    ...
    def end_match(self):
        ...
        #ADDED: Save game log when match ends (cal)
        self.save_match_log()

        tk.Button(result_window, text="Close", bg="black", fg="black",
                  command=result_window.destroy).pack(pady=10)

    #ADDED: Export the game history as a txt file (given.a path)
    def save_match_log(self):
        ...
        # Team 1 overs
        output.append("TEAM 1 INNINGS\n")
        balls = self.ball_log[0]
        for i in range(0, len(balls), 6):
            over = balls[i:i+6]
            output.append(",".join(over) + "\n")

        output.append("\n--- 2ND INNINGS START ---\n\n")

        # Team 2 overs
        output.append("TEAM 2 INNINGS\n")
        balls = self.ball_log[1]
        for i in range(0, len(balls), 6):
            over = balls[i:i+6]
            output.append(",".join(over) + "\n")

        text_to_save = "".join(output)

        # Save dialogue
        filepath = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text files", "*.txt")],
            title="Save Match Breakdown"
        )
